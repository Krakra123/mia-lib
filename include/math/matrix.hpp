...
class matrix {
  ...
  public:
    ...
    explicit constexpr matrix(T s) {
        MIA_MATRIX_LOOP_OPERATION(data[i][j] = s)
    }
    explicit constexpr matrix(T *a) {
        MIA_MATRIX_ROW_LOOP_OPERATION(data[i] = &a[i * Rows])
    }
    ...
    constexpr auto operator=(T *a) -> matrix & {
    ...
    constexpr auto operator()(const int row, const int col) -> T & {
        return data[col][row];
    }
    ...
    constexpr auto operator()(const int i) -> T & {
        return operator[](i);
    }
    ...
    constexpr auto get_row(const int row) const -> const vector<T, Cols> & {
        return data[row];
    }
    constexpr auto get_row(const int row) -> vector<T, Cols> & {
        return data[row];
    }
